...
class MySessionManager: NSObject {
    ...
    let baseURL = "http://history.muffinlabs.com/date"
    ...
    func requestGETURL(_ strURL: String, success:@escaping (JSON) -> Void, failure:@escaping (MyError) -> Void) {
        guard Utilities.sharedInstance.isNetworkConnectivityAvailable() else {
            let myError = MyError.init(errorCode: "NO_CONNECTION_ERROR", errorMessage: NSLocalizedString("No Internet connection", comment: "comment"))
            failure(myError)
            return
        }
        
        self.sessionManager.request(baseURL + strURL, headers: nil).responseJSON { (responseObject) -> Void in
            ...
            if responseObject.result.isFailure {
                if let error = responseObject.result.error {
                    let myError = MyError.init(errorCode: "NETWORK_ERROR", errorMessage: error.localizedDescription)
                    failure(myError)
                }
            }
        }
